    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-sm text-center">
        <DialogHeader className="items-center">
          ...
          <DialogDescription>{t("about.version", { version: auth?.version ?? "dev" })}</DialogDescription>
        </DialogHeader>
        {info && (
          <dl className="grid grid-cols-[auto_1fr] gap-x-4 gap-y-1 text-left text-xs">
            <dt className="text-muted-foreground">{t("about.host")}</dt><dd>{info.hostname}</dd>
            <dt className="text-muted-foreground">{t("about.system")}</dt><dd>{info.os} · {info.kernel}</dd>
            <dt className="text-muted-foreground">CPU</dt><dd className="truncate">{info.cpuModel} ({t("about.threads", { count: info.cores })})</dd>
