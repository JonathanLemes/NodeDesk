...
export function MobileFrame({ win, top, onBack }: { win: DesktopWindow; top: boolean; onBack: () => void }) {
  ...
  return (
    <WindowContext.Provider value={{ windowId: win.id, titlebar, focused: top }}>
      <div
        data-focused={top}
        className="fixed top-0 left-0 flex w-full animate-[slide-up_0.26s_cubic-bezier(.2,.8,.2,1)] flex-col bg-window text-foreground"
        style={{ zIndex: win.z, height: "var(--app-h, 100dvh)" }}
      >
        <header
        ...
        <div className="relative min-h-0 flex-1" style={{ paddingBottom: "max(env(safe-area-inset-bottom), 0px)" }}>
          <ErrorBoundary>
